...
export default function MLPredictionsPage() {
  ...
  const fetchPredictions = async () => {
    try {
      ...
      const url =
        tierFilter === 'all'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/prospect-predictions/predictions?limit=100`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/v1/prospect-predictions/predictions?tier=${tierFilter}&limit=100`;
  ...
  const getTierBadge = (tier: string) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-sm font-semibold border ${colorClasses}`}
      >
        {tier}
      </span>
    );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-wine-periwinkle text-lg">
            AI-powered prospect success predictions using 118 engineered
            features
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-wine-dark/50 rounded-lg p-4 border border-wine-periwinkle/20">
            <div className="text-wine-periwinkle text-sm mb-1">
              Total Predictions
            </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white text-sm">
                          {((prediction.confidence_score || 0) * 100).toFixed(
                            1
                          )}
                          %
                        </span>
        ...
        <div className="mt-8 bg-wine-dark/30 rounded-lg p-6 border border-wine-periwinkle/20">
          ...
          <p className="text-white/70 text-sm mb-4">
            These predictions are generated by an XGBoost machine learning model
            trained on 599 prospects with complete scouting data. The model uses
            118 engineered features across 6 categories: biographical data,
            scouting grades, MiLB performance, progression, consistency, and
            derived metrics.
          </p>
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div>
              <div className="text-wine-periwinkle font-medium">
                Model Accuracy
              </div>
