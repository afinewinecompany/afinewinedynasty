...
export function ScoutingRadar({
  ...
  const sources = useMemo(() => {
    const allSources = Array.from(
      new Set(scoutingGrades.map((grade) => grade.source))
    );
  ...
  const radarData = useMemo(() => {
    ...
    const averageGrades = {
      hit: Math.round(
        filteredGrades.reduce((sum, grade) => sum + (grade.hit || 50), 0) /
          filteredGrades.length
      ),
      power: Math.round(
        filteredGrades.reduce((sum, grade) => sum + (grade.power || 50), 0) /
          filteredGrades.length
      ),
      speed: Math.round(
        filteredGrades.reduce((sum, grade) => sum + (grade.speed || 50), 0) /
          filteredGrades.length
      ),
      field: Math.round(
        filteredGrades.reduce((sum, grade) => sum + (grade.field || 50), 0) /
          filteredGrades.length
      ),
      arm: Math.round(
        filteredGrades.reduce((sum, grade) => sum + (grade.arm || 50), 0) /
          filteredGrades.length
      ),
    };
    ...
    const tools =
      position.includes('P') || position === 'SP' || position === 'RP'
        ? [
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {radarData.map((tool, index) => (
                    ...
                    >
                      <div
                        className={`text-lg font-bold ${getGradeColor(tool.value)}`}
                      >
                        {tool.value}
                      </div>
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-blue-800 text-sm">
                        Tracking changes in scouting grades over time. Purple
                        line shows overall grade, blue line shows future value
                        projection.
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="text-center">
                        <div
                          className={`text-lg font-bold ${getGradeColor(grade.overall)}`}
                        >
                          {grade.overall}
                        </div>
                      ...
                      <div className="text-center">
                        <div
                          className={`text-lg font-bold ${getGradeColor(grade.future_value)}`}
                        >
                          {grade.future_value}
                        </div>
                      ...
                      <div className="text-center">
                        <div
                          className={`text-lg font-bold ${getGradeColor(grade.hit || 50)}`}
                        >
                          {grade.hit || 50}
                        </div>
                        <div className="text-xs text-gray-600">
                      ...
                      <div className="text-center">
                        <div
                          className={`text-lg font-bold ${getGradeColor(grade.power || 50)}`}
                        >
                          {grade.power || 50}
                        </div>
                        <div className="text-xs text-gray-600">
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                    ...
                    <div>
                      <span className="text-red-600 font-semibold">20-45:</span>{' '}
                      Below Average
                    </div>
                  </div>
                </div>
