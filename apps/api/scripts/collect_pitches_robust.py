...
async def collect_player_pitches(session, player_id, name, season, pa_count=None):
    """Collect pitch data for a single player with proper error handling"""
    conn = None
    cur = None
    total_pitches = 0
    ...
    try:
        ...
        # Check if player already has pitch data for this season
        cur.execute("""
        ...
        if existing_count > 0:
            logging.info(f"  {name}: Already has {existing_count} pitches for {season}, skipping")
            return 0
        ...
        if not game_pks:
            logging.info(f"  {name}: No {season} games to process")
            return 0
        ...
        logging.info(f"  {name} {pa_info}: Collected {total_pitches} pitches from {games_processed}/{len(game_pks)} games")
...
async def collect_batch(players: List[Tuple], batch_num: int, total_batches: int, season: int):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        for player_data in players:
            ...
            result = await collect_player_pitches(session, player_id, name, season, pa_count)
            batch_total += result
...
def main():
    print("\n" + "="*70)
    print("ROBUST PITCH COLLECTION - RESUMABLE")
    ...
    # Determine which prospects need collection
    conn = get_db_connection()
    cur = conn.cursor()

    # Get prospects that need 2025 pitch data
    cur.execute("""
    ...
    prospects_2025 = cur.fetchall()
    print(f"\nFound {len(prospects_2025)} prospects needing 2025 pitch data")
    ...
    print(f"Found {len(prospects_2024)} prospects needing 2024 pitch data")
    ...
    # Process 2025 first
    if prospects_2025:
        print(f"\n=== COLLECTING 2025 PITCH DATA ===")
        batch_size = 5  # Smaller batches for better reliability
        batches = [prospects_2025[i:i+batch_size] for i in range(0, len(prospects_2025), batch_size)]
    ...
    print(f"Total 2025 pitch records: {total_2025:,}")
