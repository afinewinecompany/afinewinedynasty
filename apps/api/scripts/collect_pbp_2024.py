"""
Collect MiLB play-by-play data for 2024 season.
...
class MiLBPBPCollector2024:
    """Collect MiLB pitch-by-pitch data for 2024 season."""
    ...
    def get_players_needing_pbp(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Get players who have 2024 game logs but no PBP data."""
...
AND p.mlb_player_id IS NOT NULL
    ...
    async def find_player_games_2024(self, player_id: int) -> List[Dict[str, Any]]:
        """Find all 2024 games for a player across all MiLB levels."""
        all_games = {}  # Use dict to dedupe by game_pk

        # Query each MiLB sport level
        for sport_id, level_name in self.MILB_SPORT_IDS.items():
            url = f"{self.BASE_URL}/people/{player_id}/stats?stats=gameLog&season=2024&group=hitting,pitching&sportId={sport_id}"
            ...
            with sync_engine.begin() as conn:
                for play_idx, play in enumerate(all_plays):
                    ...
                    insert_query = text("""
                        ...
                        VALUES
                        (:player_id, :game_pk, :game_date, 2024, :level, :at_bat_index,
                         :inning, :half_inning, :event_type, :event_type_desc, :description,
                         :launch_speed, :launch_angle, :total_distance, :trajectory, :hardness,
    ...
    async def collect_player_pbp(self, player: Dict[str, Any]) -> int:
        """Collect all 2024 PBP data for a single player."""
        ...
        # Find all 2024 games
        games = await self.find_player_games_2024(player_id)

        if not games:
            logger.info(f"    No 2024 games found via API")
            return 0

        logger.info(f"    Found {len(games)} games in 2024")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Collect MiLB play-by-play data for 2024 season"
    )
    parser.add_argument(
        '--limit',
        type=int,
    ...
    logger.info("="*80)
    logger.info("MiLB Play-by-Play Collection for 2024 Season")
    logger.info("="*80)

    try:
        async with MiLBPBPCollector2024() as collector:
            # Ensure table exists
            ...
            logger.info(f"Found {len(players)} players needing PBP data for 2024")
            logger.info("")
