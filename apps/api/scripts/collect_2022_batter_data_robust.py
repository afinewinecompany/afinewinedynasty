...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/2022_batter_collection.log'),
...
async def collect_player_data(session, conn, player_id, name, season):
    """Collect all 2022 data for a single player"""
    all_games = []
...
async def main():
    ...
    # Get batters missing 2022 data (properly exclude ALL pitcher positions)
    cur.execute("""
        SELECT
            p.mlb_player_id,
            p.name,
        ...
        AND (
            NOT EXISTS (
                ...
                WHERE mpa.mlb_player_id = p.mlb_player_id::INTEGER AND mpa.season = 2022
            )
            OR NOT EXISTS (
                SELECT 1 FROM milb_batter_pitches mbp
                WHERE mbp.mlb_batter_id = p.mlb_player_id::INTEGER AND mbp.season = 2022
            )
        )
    ...
    logging.info("")
    logging.info("="*80)
    logging.info("ROBUST 2022 BATTER DATA COLLECTION - WITH RETRY LOGIC")
    logging.info("="*80)
    logging.info(f"\nFound {len(prospects)} non-pitcher prospects missing 2022 data")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for idx, (player_id, name, org, rank) in enumerate(prospects):
            try:
                ...
                pas, pitches = await collect_player_data(session, conn, player_id, name, 2022)
