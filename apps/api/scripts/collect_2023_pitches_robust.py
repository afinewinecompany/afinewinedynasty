...
def init_connection_pool():
    ...
    try:
        ...
        logging.info("Database connection pool initialized for 2023 collection")
        ...
        for event in play_events:
            if event.get('isPitch'):
                ...
                pitch = {
                    ...
                    'game_pk': game_pk,
                    'game_date': game_date,
                    'season': 2023,  # Fixed to 2023 for this script
...
async def collect_player_pitches(session, player_id, name, pa_count=None):
    """Collect 2023 pitch data for a single player with proper error handling"""
    conn = None
    cur = None
    total_pitches = 0
    ...
    try:
        ...
        # Check if player already has 2023 pitch data
        cur.execute("""
            SELECT COUNT(*) FROM milb_batter_pitches
            WHERE mlb_batter_id = %s AND season = 2023
        """, (player_id,))

        existing_count = cur.fetchone()[0]
        if existing_count > 0:
            logging.info(f"  {name}: Already has {existing_count} pitches for 2023, skipping")
            return 0

        # Get game PKs from PBP table for 2023
        cur.execute("""
            SELECT DISTINCT game_pk, game_date
            FROM milb_plate_appearances
            WHERE mlb_player_id = %s AND season = 2023
            ORDER BY game_date
            LIMIT 150
        """, (player_id,))
        ...
        if not game_pks:
            logging.info(f"  {name}: No 2023 games to process")
            return 0
        ...
        logging.info(f"  {name} {pa_info}- 2023: Collected {total_pitches} pitches from {games_processed}/{len(game_pks)} games")

    except Exception as e:
        logging.error(f"Error collecting 2023 data for {name}: {e}")
...
async def collect_batch(players: List[Tuple], batch_num: int, total_batches: int):
    """Collect 2023 pitch data for a batch of players"""
    logging.info(f"\n=== 2023 BATCH {batch_num}/{total_batches}: {len(players)} players ===")
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        logging.info(f"  2023 Batch {batch_num} complete: {batch_total} total pitches")
        return batch_total

def main():
    print("\n" + "="*70)
    print("ROBUST 2023 PITCH COLLECTION - PROSPECTS ONLY")
    ...
    # Get prospects that need 2023 pitch data
    conn = get_db_connection()
    cur = conn.cursor()

    # First, check how much 2023 data we already have
    cur.execute("""
        SELECT COUNT(*) as total_records,
               COUNT(DISTINCT mlb_batter_id) as unique_players
        FROM milb_batter_pitches
        WHERE season = 2023
    """)
    total_2023, unique_2023 = cur.fetchone()
    print(f"\nCurrent 2023 data: {total_2023:,} records from {unique_2023} players")
    ...
    prospects_2023 = cur.fetchall()
    print(f"\nFound {len(prospects_2023)} prospects needing 2023 pitch data")

    if len(prospects_2023) > 0:
        print(f"\nTop 10 prospects by PA count:")
        for player_id, name, org, pa_count in prospects_2023[:10]:
    ...
    # Process 2023 data
    if prospects_2023:
        print(f"\n=== COLLECTING 2023 PITCH DATA ===")
        batch_size = 5  # Smaller batches for better reliability
        batches = [prospects_2023[i:i+batch_size] for i in range(0, len(prospects_2023), batch_size)]
        ...
        for i, batch in enumerate(batches, 1):
            try:
                ...
                if i % 5 == 0:
                    ...
                    conn = get_db_connection()
                    cur = conn.cursor()
                    cur.execute("SELECT COUNT(*) FROM milb_batter_pitches WHERE season = 2023")
                    total_2023_now = cur.fetchone()[0]
                    cur.close()
                    return_connection(conn)

                    print(f"\n[PROGRESS] Completed {i}/{len(batches)} batches")
                    print(f"  Total 2023 pitches in DB: {total_2023_now:,}")
        ...
        print("\n✅ All prospects already have 2023 pitch data!")
    ...
    cur.execute("SELECT COUNT(*) FROM milb_batter_pitches WHERE season = 2023")
    final_2023 = cur.fetchone()[0]

    cur.execute("SELECT COUNT(DISTINCT mlb_batter_id) FROM milb_batter_pitches WHERE season = 2023")
    unique_players = cur.fetchone()[0]

    # Get top prospects with 2023 data
    cur.execute("""
        ...
        WHERE mbp.season = 2023
        GROUP BY p.name, p.organization
        ORDER BY pitch_count DESC
        LIMIT 5
    """)

    print(f"\n=== TOP PROSPECTS WITH 2023 PITCH DATA ===")
    for name, org, count in cur.fetchall():
        print(f"  {name:30} ({org:3}) - {count:,} pitches")
    ...
    print(f"Total 2023 pitch records: {final_2023:,}")
    print(f"New records added: {total_collected:,}")
    print(f"Unique players with 2023 data: {unique_players:,}")
