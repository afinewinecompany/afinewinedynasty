...
async def get_batter_games(session: aiohttp.ClientSession, batter_id: int) -> List[Dict]:
    """Get all games for a batter in 2025"""
...
async def collect_batter_data(session: aiohttp.ClientSession, batter_id: int, batter_name: str, org: str) -> int:
    """Collect all 2025 pitches for a batter"""
...
def main():
    print("=" * 80)
    print("BATTER PITCH DATA COLLECTION - 2025 SEASON (MISSING DATA ONLY)")
    ...
    try:
        ...
        # Get batters with 2025 PAs but missing pitch data
        cur.execute("""
        ...
        print(f"\nFound {len(missing_batters)} batters missing 2025 pitch data")
        ...
        print(f"Current 2025: {batters} batters, {pitches:,} pitches")

        print(f"\n=== STARTING 2025 COLLECTION FOR MISSING BATTERS ===\n")
        ...
        for i, batch in enumerate(batches, 1):
            ...
            if i % 10 == 0:
                ...
                """)
                batters, pitches = cur.fetchone()
                print(f"  2025 in DB: {batters} batters, {pitches:,} pitches")

        print("\n" + "=" * 80)
        print("2025 BATTER COLLECTION COMPLETE")
        print("=" * 80)
        ...
        """)
        batters, pitches = cur.fetchone()
        print(f"\n2025 Final: {batters} batters, {pitches:,} pitches")
