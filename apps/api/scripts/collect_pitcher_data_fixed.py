...
async def get_pitcher_games(session: aiohttp.ClientSession, pitcher_id: int, season: int) -> List[Dict]:
    """Get all games for a pitcher in a season"""
...
async def collect_game_pitches(session: aiohttp.ClientSession, pitcher_id: int, game_info: Dict, season: int) -> List[Tuple]:
        ...
        async with session.get(url, timeout=30) as response:
            ...
            for play in all_plays:
                ...
                for i, event in enumerate(play_events):
                    ...
                    # If ball in play, try to add hit data
                    if details.get('isInPlay'):
                        hit_data = event.get('hitData', {})
                        if hit_data:
                            # Update the last 5 fields before created_at
...
async def collect_pitcher_all_seasons(session: aiohttp.ClientSession, pitcher_id: int, pitcher_name: str, org: str) -> int:
    """Collect all pitches for a pitcher across all seasons"""
    ...
    try:
        ...
        for season in SEASONS:
            ...
            logger.info(f"  {pitcher_name} ({org}) - {season}: {len(games)} games, {expected_pitches} expected pitches")
            ...
            for game_info in games:
                ...
                if game_pitches:
                    ...
                    try:
                        ...
                        """, game_pitches)
                        conn.commit()
                        season_collected += len(game_pitches)
                    except Exception as e:
                        logger.error(f"    DB error for {pitcher_name}: {str(e)}")
                        conn.rollback()

            if season_collected > 0:
                logger.info(f"    -> Collected {season_collected} pitches")
...
def main():
    print("=" * 80)
    print("FIXED PITCHER DATA COLLECTION - ALL SEASONS")
    ...
    try:
        ...
        print(f"\nFound {len(all_pitchers)} pitchers to process")

        # Current status
        cur.execute("""
            SELECT season, COUNT(DISTINCT mlb_pitcher_id), COUNT(*)
        ...
        grand_total = 0
        for season, pitchers, pitches in cur.fetchall():
            print(f"\n{season} Final: {pitchers} pitchers, {pitches:,} pitches")
            grand_total += pitches

        print(f"\nGRAND TOTAL: {grand_total:,} pitcher pitches")
