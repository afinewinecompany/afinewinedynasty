...
async def check_data_scope():
    ...
    if game_log_players > prospect_count * 2:
        print("\n✅ CONCLUSION: We have COMPREHENSIVE MiLB data, not just our prospects!")
        print("   This means we CAN calculate league-wide age-relative-to-level adjustments!")
    else:
        print("\n⚠️  CONCLUSION: We may only have prospect-specific data")
    ...
    if missing > 0:
        print(f"\n⚠️  {missing} prospects NOT found in game logs")
    ...
    if non_prospects:
        ...
        print(f"\n✅ We have {len(non_prospects)} non-prospect players in the sample")
        print("   This confirms LEAGUE-WIDE data collection!")
    else:
        print("⚠️  No non-prospect players found - we may only have prospect data")
    ...
    if aa_2024 > 100:
        print("✅ Sufficient sample size for age-relative-to-level calculations")
        print("   We can calculate percentile rankings by age at each level!")
    else:
        print("⚠️  Small sample size - age-relative calculations may be noisy")
    ...
    if age_check['players_with_birth_date'] < age_check['players_in_logs'] / 10:
        print("\n⚠️  MAJOR LIMITATION: We only have birth dates for our tracked prospects!")
        ...
        print("\n✅ We have good age coverage across all players")
    ...
    if game_log_players > prospect_count * 5:
        ...
        print("   - Ratio: {:.1f}x".format(game_log_players / prospect_count))
    else:
        print("\n⚠️  DATA SCOPE: PROSPECT-FOCUSED")
        print(f"   - We have {game_log_players:,} players vs {prospect_count:,} prospects")

    print("\nFOR AGE-RELATIVE-TO-LEVEL ADJUSTMENTS:")

    if age_check['players_with_birth_date'] < age_check['players_in_logs'] / 10:
        print("   ⚠️  LIMITED: Birth dates only available for tracked prospects")
        print("   📋 ACTION NEEDED:")
