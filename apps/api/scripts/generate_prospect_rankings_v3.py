...
class UnifiedProspectRankingSystem:
    ...
    async def load_hitter_data(self) -> pd.DataFrame:
        ...
        async with engine.begin() as conn:
            result = await conn.execute(text("""
                ...
                statcast AS (
                    SELECT
                        mlb_player_id,
                        AVG(avg_ev) as avg_exit_velo,
                        MAX(max_ev) as max_exit_velo,
                        AVG(ev_90th) as exit_velo_90th,
                        AVG(hard_hit_pct) as hard_hit_pct,
                        AVG(avg_la) as avg_launch_angle,
                        AVG(avg_la_hard) as avg_la_hard_contact,
                        AVG(fb_ev) as flyball_exit_velo,
                ...
                SELECT
                    ...
                    sc.hard_hit_pct,
                    sc.avg_launch_angle,
                    sc.avg_la_hard_contact,
                    sc.flyball_exit_velo,
                    sc.barrel_pct
        ...
        df = pd.DataFrame(rows, columns=[
            ...
            'mlb_ab', 'mlb_pa',
            'avg_exit_velo', 'max_exit_velo', 'exit_velo_90th', 'hard_hit_pct',
            'avg_launch_angle', 'avg_la_hard_contact', 'flyball_exit_velo', 'barrel_pct'
        ])
