...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/2021_pitcher_collection_debug.log'),
        logging.StreamHandler()
    ]
)

# Set console to INFO, file to DEBUG
...
async def collect_pitcher_data(session, conn, pitcher_id, name, position, season):
    """Collect all 2021 pitching data for a single pitcher"""
    all_games = []
    ...
    logging.info(f"  -> Starting play-by-play collection for {len(all_games)} games...")
    for i, game_info in enumerate(all_games):
        game_pk = game_info['game']['gamePk']
        game_date = game_info.get('date', 'unknown')

        if i < 3:  # Log first 3 game PKs to verify we're getting them
            logging.info(f"    Processing game {i+1}: PK={game_pk}, Date={game_date}")
        ...
        if pbp_data:
            ...
            games_without_pbp += 1
            logging.debug(f"      Game {game_pk}: No play-by-play data returned from API")

        # Progress update every 10 games
        if (i + 1) % 10 == 0:
            logging.info(f"  -> Processed {i+1}/{len(all_games)} games ({total_pitches} pitches, {games_with_pbp} with PBP, {games_without_pbp} without)")
    ...
    logging.info(f"  -> PBP Summary: {games_with_pbp} games with data, {games_without_pbp} games without data")

    if games_with_pbp > 0 and total_pitches == 0:
        logging.warning(f"  -> WARNING: Got play-by-play data but extracted 0 pitches! Check debug logs.")
...
async def main():
    ...
    # Get pitchers missing 2021 data
    cur.execute("""
        SELECT
            p.mlb_player_id,
            p.name,
        ...
        AND (
            NOT EXISTS (
                ...
                WHERE mpa.mlb_player_id = p.mlb_player_id::INTEGER AND mpa.season = 2021
            )
            OR NOT EXISTS (
    ...
    logging.info("")
    logging.info("="*80)
    logging.info("ROBUST 2021 PITCHER DATA COLLECTION - WITH RETRY LOGIC")
    logging.info("="*80)
    logging.info(f"\nFound {len(pitchers)} pitchers needing 2021 data collection")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for idx, (player_id, name, position, org, rank) in enumerate(pitchers):
            try:
                ...
                games, pitches = await collect_pitcher_data(session, conn, player_id, name, position, 2021)
