...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/2022_pitcher_collection.log'),
...
async def collect_pitcher_data(session, conn, pitcher_id, name, position, season):
    """Collect all 2022 pitching data for a single pitcher"""
    all_games = []
...
async def main():
    ...
    # Get pitchers missing 2022 data
    cur.execute("""
        SELECT
            p.mlb_player_id,
            p.name,
        ...
        AND (
            ...
            OR NOT EXISTS (
                ...
                WHERE mpp.mlb_pitcher_id = p.mlb_player_id::INTEGER AND mpp.season = 2022
            )
        )
    ...
    logging.info("")
    logging.info("="*80)
    logging.info("ROBUST 2022 PITCHER DATA COLLECTION - WITH RETRY LOGIC")
    logging.info("="*80)
    logging.info(f"\nFound {len(pitchers)} pitchers needing 2022 data collection")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for idx, (player_id, name, position, org, rank) in enumerate(pitchers):
            try:
                ...
                games, pitches = await collect_pitcher_data(session, conn, player_id, name, position, 2022)
