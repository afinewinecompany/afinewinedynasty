...
async def collect_player_pbp(session, player_id, name, conn):
    ...
    # Check if player already has 2024 data
    cur.execute("""
    ...
    if cur.fetchone()[0] > 0:
        logging.info(f"  {name}: Already has 2024 data, skipping")
        cur.close()
        return 0

    # Fetch game log
    games = await fetch_game_log(session, player_id, 2024)

    if not games:
        logging.info(f"  {name}: No 2024 games found")
        cur.close()
        return 0
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for player_id, name, team in players:
            ...
            if len(tasks) >= 5:
                ...
                logging.info(f"  Batch progress: {batch_total} PAs collected")
                tasks = []
        ...
        if tasks:
            results = await asyncio.gather(*tasks)
            batch_total += sum(results)
            logging.info(f"  Batch complete: {batch_total} total PAs")
...
def main():
    print("\n" + "="*70)
    print("COMPREHENSIVE 2024 PBP COLLECTION")
    print(f"Started: {datetime.now()}")
    print("="*70)

    # Load prospects needing PBP
    prospects = []
    with open('needs_2024_pbp.txt', 'r') as f:
        for line in f:
    ...
    for i, batch in enumerate(batches, 1):
        ...
        if i % 5 == 0:
            ...
            print(f"  Total 2024 PBP records: {total:,}")
            print(f"  Collection rate: {rate:.1f} PAs/sec")
    ...
    print(f"Total 2024 PBP records: {final_total:,}")
