...
async def get_pitcher_games(session: aiohttp.ClientSession, pitcher_id: int) -> List[Dict]:
    """Get all games for a pitcher in 2024"""
...
async def collect_pitcher_data(session: aiohttp.ClientSession, pitcher_id: int, pitcher_name: str, org: str) -> int:
    """Collect all 2024 pitches for a pitcher"""
...
def main():
    ...
    try:
        ...
        print(f"\nFound {len(all_pitchers)} pitchers to process for 2024")
        ...
        print(f"Current 2024: {pitchers} pitchers, {pitches:,} pitches")

        print(f"\n=== STARTING 2024 COLLECTION ===\n")
        ...
        for i, batch in enumerate(batches, 1):
            ...
            if i % 10 == 0:
                ...
                """)
                pitchers, pitches = cur.fetchone()
                print(f"  2024 in DB: {pitchers} pitchers, {pitches:,} pitches")
        ...
        """)
        pitchers, pitches = cur.fetchone()
        print(f"\n2024 Final: {pitchers} pitchers, {pitches:,} pitches")
