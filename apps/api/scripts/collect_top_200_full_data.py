...
async def collect_full_player_data(session, player_id, name, rank, conn):
    """Collect both PBP and Pitch data for a player from scratch"""
    cur = conn.cursor()
    ...
    # Fetch game log from MLB Stats API
    games = await fetch_game_log(session, player_id, 2025)

    if not games:
        logging.info(f"  -> No 2025 games found in MLB Stats API")
        cur.close()
        return {'pbp': 0, 'pitches': 0}

    game_pks = list(set([g.get('game', {}).get('gamePk') for g in games if 'game' in g]))
    logging.info(f"  -> Found {len(game_pks)} games from Stats API")
...
async def main():
    """Main collection process for prospects missing BOTH PBP and Pitch data"""
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()

    logging.info("\n" + "="*80)
    logging.info("TOP 200 PROSPECTS - FULL DATA COLLECTION (MISSING BOTH PBP AND PITCH)")
    logging.info("="*80)

    # Get top 200 prospects missing BOTH PBP and Pitch data
    cur.execute("""
        SELECT
            p.mlb_player_id,
            p.name,
    ...
    logging.info(f"\nFound {len(missing_prospects)} top-200 prospects missing ALL 2025 data")

    if not missing_prospects:
        logging.info("\n>>> All top 200 prospects have at least some 2025 data!")
        conn.close()
        return

    # Display list
    logging.info(f"\n{'Rank':<6} {'Name':<30} {'Org':<5} {'Pos':<5}")
    logging.info("-" * 50)
    for mlb_id, name, org, pos, rank in missing_prospects:
        logging.info(f"{rank:<6} {name:<30} {org:<5} {pos or 'N/A':<5}")

    logging.info("\n" + "="*80)
    logging.info("STARTING FULL DATA COLLECTION")
    logging.info("="*80)
    ...
    async with aiohttp.ClientSession() as session:
        for i, (mlb_id, name, org, pos, rank) in enumerate(missing_prospects, 1):
            try:
                result = await collect_full_player_data(session, int(mlb_id), name, rank, conn)
            ...
            if i % 3 == 0 or i == len(missing_prospects):
                logging.info(f"\n{'='*60}")
    ...
    # Final summary
    logging.info("\n" + "="*80)
    logging.info("FULL DATA COLLECTION COMPLETE")
    logging.info("="*80)
