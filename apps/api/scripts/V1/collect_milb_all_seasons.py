"""
Collect Minor League Baseball (MiLB) game stats for 2021-2025 seasons.
Uses MLB StatsAPI wrapper to collect comprehensive game-by-game stats.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'milb_collection_2021_2025.log'),
...
class MiLBSeasonCollector:
    """Collector for Minor League Baseball statistics 2021-2025."""

    def __init__(self, checkpoint_file: str = 'V1/collection_checkpoint.json'):
        ...
        # Minor League sport IDs (reorganized in 2021)
        self.sport_ids = {
            'Triple-A': 11,
            'Double-A': 12,
            'High-A': 13,
            'Single-A': 14,
            'Rookie': 16,
            'Complex': 17  # Complex leagues (previously short-season)
        }
    ...
    def get_season_teams(self, season: int) -> List[Dict]:
        """Get all minor league teams for a season."""
        all_teams = []

        for level_name, sport_id in self.sport_ids.items():
            try:
                ...
                for team in teams:
                    team['level'] = level_name
                    team['sport_id'] = sport_id
                all_teams.extend(teams)

                logger.info(f"Found {len(teams)} teams for {season} {level_name}")
                time.sleep(0.5)  # Rate limiting

            except Exception as e:
                logger.error(f"Error fetching teams for {season} {level_name}: {e}")
    ...
    async def run_collection(self):
        ...
        logger.info("Starting MiLB data collection for 2021-2025")
