...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/2023_batter_collection.log'),
...
async def collect_player_data(session, conn, player_id, name, season):
    """Collect all 2023 data for a single player"""
    all_games = []
...
async def main():
    ...
    # Get batters missing 2023 data (properly exclude ALL pitcher positions)
    cur.execute("""
        SELECT
            p.mlb_player_id,
            p.name,
        ...
        AND (
            ...
            OR NOT EXISTS (
                ...
                WHERE mbp.mlb_batter_id = p.mlb_player_id::INTEGER AND mbp.season = 2023
            )
        )
    ...
    logging.info("")
    logging.info("="*80)
    logging.info("ROBUST 2023 BATTER DATA COLLECTION - WITH RETRY LOGIC")
    logging.info("="*80)
    logging.info(f"\nFound {len(prospects)} non-pitcher prospects missing 2023 data")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for idx, (player_id, name, org, rank) in enumerate(prospects):
            try:
                ...
                pas, pitches = await collect_player_data(session, conn, player_id, name, 2023)
