#!/usr/bin/env python3
"""
Collect Birth Dates for All MiLB Players by Season - FIXED VERSION
===================================================================
Updates prospects table with birth dates from MLB Stats API.
Can run concurrently for different seasons.

Usage:
    python collect_birth_dates_fixed.py --season 2024
    python collect_birth_dates_fixed.py --season 2025
"""
...
class BirthDateCollector:
        ...
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    # Don't log every non-200 status
                    return None
    ...
    def parse_height_to_inches(self, height_str: Optional[str]) -> Optional[int]:
    ...
    async def update_prospect_birth_date(self, prospect_id: int, player_id: int, bio_data: Dict) -> bool:
        ...
        try:
            # Parse height in Python to avoid SQL type issues
            height_inches = self.parse_height_to_inches(bio_data.get('height'))
            ...
            print(f"[{self.season}][ERROR] Failed to update prospect {prospect_id}: {e}")
            return False
